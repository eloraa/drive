...
export const Signin = () => {
  ...
  return (
    <div
      className={cn(
        'rounded-lg border border-transparent p-4 transition-colors',
        isSend && 'border-border',
      )}
    >
      <div>
        <figure className={cn('mx-auto h-20 w-20', isSend && 'grayscale')}>
          <Image
            src='/cloud.gif'
            alt='Drive'
            width='80'
      ...
      <div className='flex gap-8 overflow-hidden p-2'>
        ...
        <div
          className={cn(
            'w-full min-w-full pt-2 transition-transform',
            !isEmail && 'translate-x-full',
          )}
          ref={(el) => {
            if (!el) return;
            ...
            <div className='flex h-full w-full items-center justify-center overflow-hidden border-t'>
              <div className='flex h-full w-full items-center justify-center rounded-lg bg-background px-6 py-4'>
                <div>
                  ...
                  <h1 className='mx-auto max-w-40 text-sm'>
                  ...
                  <div className='pt-6'>
                    <div className='group flex items-center gap-2 rounded-md bg-[#131313] px-2 py-1 pl-3 font-mono text-sm text-[#3cdca6]'>
                      <p className='text-current transition-colors group-hover:text-white'>
                        {email}
                      </p>
              ...
              <Form {...form}>
                ...
                >
                  ...
                  <div className='relative overflow-hidden p-1'>
                    <div
                      className={cn(
                        'absolute inset-0 z-10 flex -translate-y-full items-center justify-center transition-transform',
                        loading && 'translate-y-0',
                      )}
                    >
                      <Loader className='h-6 w-6 *:stroke-[20]' />
                    </div>
                    <Button
                      className={cn(
                        'mx-auto flex w-full max-w-32 items-center justify-between gap-2',
                        loading && 'translate-y-[calc(100%+8px)]',
                      )}
                    >
                      Sign In
                      <div className='h-4 w-4'>
